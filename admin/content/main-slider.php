...
if (!empty($_POST["contentSubmit"])) {
    ...
    $xml = new SimpleXMLElement('<sliderimages/>');
    ...
    $xml->saveXML(getenv("DOCUMENT_ROOT") . "/config/" . $_POST["filename"]);
    $alertMsg = "Slider images were successfully updated. ";
}
if (!empty($errorMsg)) {
...
$html.= "<form action='main-slider' method='post' enctype='multipart/form-data'>"
      .     "<h1>Main Slider</h1>"
      .     getSliderOptions()
      .     "<input name='filename' type='hidden' value='sliderConfig.xml'>"
